...
def train_model(model, epoch_num, batch_size, out_model=None, pairwise=True, d=14, MAP=-1, model_path='tmp/tmp_best2.h5'):
    ...
    tmp_best = model_path
    best_epoch = [0]
    train_data = np.load('data/data_rpca/x_train_rpca.npy').tolist()
    test_data = np.load('data/data_rpca/x_test_rpca.npy').tolist()
    valid_data = np.load('data/data_rpca/x_val_rpca.npy').tolist()
    train_labels = np.load('data/data_rpca/y_train_rpca.npy').tolist()
    test_labels = np.load('data/data_rpca/y_test_rpca.npy').tolist()
    valid_labels = np.load('data/data_rpca/y_val_rpca.npy').tolist()
    ...
    class LossHistory(Callback):
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            if compute_all or np.sum(_val_result) > np.sum(best_val_accuracy[0]):
                ...
                np.save('output/pairwise/data_rpca/y_train_rpca.npy', np.array(self.train_labels))
                np.save('output/pairwise/data_rpca/x_validate_rpca.npy', np.array(_validate))
                np.save('output/pairwise/data_rpca/y_val_rpca.npy', np.array(self.validate_labels))
                #np.save('output/pairwise/x_test.npy', np.array(self.test_pred))
                np.save('output/pairwise/data_rpca/y_test_rpca.npy', np.array(self.test_labels))
    ...
    if pairwise is True:
        ...
        H = model.fit(train_data + train_labels, train_labels[0], batch_size=batch_size, epochs=epoch_num, shuffle=False, callbacks=[history], verbose=1)
        if isComputeLoss:
            import scipy.io as sio
    ...
    train_list = []
    val_list = []
    val = np.load('output/pairwise/data_rpca/x_validate_rpca.npy')
    ...
    np.save('output/pairwise/data_rpca/x_test_rpca.npy', np.array(test_list))
    np.save('output/pairwise/data_rpca/x_train_rpca.npy', np.array(train_list))
    np.save('output/pairwise/data_rpca/x_val_rpca.npy', np.array(val_list))
