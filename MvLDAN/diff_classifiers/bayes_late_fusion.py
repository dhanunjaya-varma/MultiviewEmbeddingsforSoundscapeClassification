...
gnb_bg = GaussianNB()
gnb_bg.fit(x_train_bg, y_train)

gnb_fg = GaussianNB()
gnb_fg.fit(x_train_fg, y_train)

y_pred = []

for i in range(x_test_bg.shape[0]):
	pred_bg = gnb_bg.predict_proba(x_test_bg[i].reshape(1,-1))
	pred_fg = gnb_fg.predict_proba(x_test_fg[i].reshape(1,-1))
