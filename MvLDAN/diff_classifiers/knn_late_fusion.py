...
neigh_bg = KNeighborsClassifier(n_neighbors=1, metric='cosine')
neigh_bg.fit(x_train_bg, y_train)

neigh_fg = KNeighborsClassifier(n_neighbors=1, metric='cosine')
neigh_fg.fit(x_train_fg, y_train)
y_pred = []

for i in range(x_test_bg.shape[0]):
	pred_bg = neigh_bg.predict_proba(x_test_bg[i].reshape(1,-1))
	pred_fg = neigh_fg.predict_proba(x_test_fg[i].reshape(1,-1))
