...
def th_MvLDAN_test_test(train, train_labels, test, test_labels, d_range=9, MAP=None):
    n_view = len(train)
    train_labels_tmp = []
    test_labels_tmp = []
    train_tmp = []
    ...
    for d in d_range:
        ...
        for i in range(n_view):
            test_list.append(np.dot((test[i] - ms[0][i]) / ms[1][i], W[i][:, 0:d]))
        tmp = th_multi_test_test(test_list, test_labels, MAP)
...
def th_multi_test_test(data, data_labels, MAP=None):
    n_view = len(data)
    res = np.zeros([n_view, n_view])
    if MAP is None:
        for i in range(n_view):
