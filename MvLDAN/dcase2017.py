...
def train_model(model, epoch_num, batch_size, out_model=None, pairwise=True, d=14, MAP=-1, model_path='tmp/tmp_best2.h5'):
    ...
    tmp_best = model_path
    best_epoch = [0]
    train_data = np.load('data/DCASE2017/x_train_auto_bg.npy').tolist()
    test_data = np.load('data/DCASE2017/x_test_auto_bg.npy').tolist()
    valid_data = np.load('data/DCASE2017/x_val_auto_bg.npy').tolist()
    train_labels = np.load('data/DCASE2017/y_train_auto_bg.npy').tolist()
    test_labels = np.load('data/DCASE2017/y_test_auto_bg.npy').tolist()
    valid_labels = np.load('data/DCASE2017/y_val_auto_bg.npy').tolist()
    ...
    class LossHistory(Callback):
        ...
        def on_epoch_end(self, epoch, logs=None):
            ...
            if compute_all or np.sum(_val_result) > np.sum(best_val_accuracy[0]):
                ...
                np.save('output/pairwise/y_train.npy', np.array(self.train_labels))
                np.save('output/pairwise/x_validate.npy', np.array(_validate))
                np.save('output/pairwise/y_val.npy', np.array(self.validate_labels))
                #np.save('output/pairwise/x_test.npy', np.array(self.test_pred))
                np.save('output/pairwise/y_test.npy', np.array(self.test_labels))
    ...
    for i in range(1, 100):
        ...
        if not os.path.exists(file_name):
            ...
            train_list = []
            val_list = []
            val = np.load('output/pairwise/x_validate.npy')
            ...
            np.save('output/pairwise/x_test.npy', np.array(test_list))
            np.save('output/pairwise/x_train.npy', np.array(train_list))
            np.save('output/pairwise/x_val.npy', np.array(val_list))
            if len(train_labels) == 14:
                sio.savemat(file_name, {'view0': test_list[0], 'view1': test_list[1], 'view0_lab': test_labels[0], 'view1_lab': test_labels[1]})
            else:
